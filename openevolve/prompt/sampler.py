    ...
    def _identify_improvement_areas(
        ...
        if len(previous_programs) >= 2:
            ...
            for metric, value in metrics.items():
                ...
                # if metric starts with -, then it is a metric that we want to minimize
                ...
                for attempt in recent_attempts:
                    attempt_value = -attempt["metrics"].get(metric, 0) if metric.startswith("-") else attempt["metrics"].get(metric, 0)
