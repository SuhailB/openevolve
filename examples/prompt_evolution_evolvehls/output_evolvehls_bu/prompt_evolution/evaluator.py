...
# Stage-based evaluation for cascade evaluation, first stage is to run csim only
def evaluate_stage1(program_path):
    ...
    try:
        # Step 1: Evaluate C-simulation
        csim_result = evaluate_csim(program_path, build_dir)
        
        # If C-simulation failed, return early
        if csim_result.metrics["success"] == 0.0:
            print("❌ C-simulation failed, skipping synthesis")
            return csim_result
